<template>
  <div class="widget-container" v-model="validation">
    ...
    >
      <div
        class="wimg"
        :style="'background:url(' + item[mapping.image] + ')'"
        @click="product_select(item)"
      >
        <div
          v-if="options.salebox && parseFloat(item[mapping.sale_price]) > 0"
          class="widget-sale"
        >{{Discount(item[mapping.price],item[mapping.sale_price])}}% OFF</div>
      </div>
      <div
        class="widget-title"
      ...
      <div class="widget-card-footer">
        ...
        <div class="widget-price">
          ...
          >{{ options.currency }}{{ fullprice(item[mapping.price],item[mapping.sale_price]) }}</div>
          <div
            class="product-sale"
            :style="{ color: options.sale_color }"
            v-if="parseFloat(item[mapping.sale_price]) > 0"
          >
            {{ options.currency }}{{ item[mapping.sale_price].toFixed(2) }}
            <span
              :style="{color:options.price_color}"
              class="fullprice"
            >{{item[mapping.price].toFixed(2)}}</span>
          </div>
        </div>
      </div>
    </div>
